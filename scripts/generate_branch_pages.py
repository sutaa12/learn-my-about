#!/usr/bin/env python3
"""Generate a GitHub Pages hub that indexes every branch containing an index.html."""
...
def sanitize_href(target: str) -> str:
    """Only allow safe URL schemes and relative paths."""
...
def render_inline(text: str) -> str:
    ...
    return LINK_RE.sub(
        lambda m: f'<a href="{sanitize_href(m.group(2))}">{m.group(1)}</a>',
        escaped,
    )
...
# ---------------------------------------------------------------------------
# File export (tar-based for non-worktree branches)
# ---------------------------------------------------------------------------
...
def safe_extract_tar(archive_path: Path, destination: Path) -> None:
    ...
    with tarfile.open(archive_path) as archive:
        for member in archive.getmembers():
            ...
            if (
                destination_root not in resolved_member.parents
                and resolved_member != destination_root
            ):
                raise ValueError(
                    f"Refused to extract outside destination: {member.name}"
                )
            archive.extract(member, destination)


def export_branch_files(
    branch_name: str, source: str, destination: Path, output_dir: Path
) -> None:
    if source == "worktree":
        ...
        for raw_path in tracked.split("\0"):
            ...
            if output_relative and (
                raw_path == output_relative
                or raw_path.startswith(f"{output_relative}/")
            ):
                continue
    ...
    try:
        run_git(
            ["archive", "--format=tar", "-o", str(tmp_path), source],
            capture_output=False,
        )
        safe_extract_tar(tmp_path, destination)
    finally:
...
def render_tree(tree: dict[str, dict[str, object]], *, depth: int = 0) -> str:
    ...
    for key in sorted(tree):
        ...
        if entry is None:
            items.append(
                f'<li class="branch-node" role="treeitem">'
                f'<div class="branch-node__label">{html.escape(key)}</div>'
                f"{children_html}</li>"
            )
            continue
        ...
        items.append(
            ...
            '    <a class="branch-card__link" href="{link}">'
            "このブランチの index.html を開く</a>\n"
            "  </article>\n"
            "  {children_html}\n"
            "</li>".format(
...
def write_top_page(
    output_dir: Path, repository: str, entries: list[BranchEntry]
) -> None:
    ...
    if not entries:
        empty_state = (
            ...
            "<p>各ブランチのルートに <code>index.html</code> を置くと、"
            "このトップページに自動で追加されます。</p></section>"
        )
  ...
  <body>
    <main class="page">
      <section class="hero">
        ...
        <p>
          このトップページは、各ブランチのルートにある <code>index.html</code> と任意の
          <code>description.md</code> を集めて、GitHub Pages 向けの静的サイトとして公開する仕組みで
          生成されています。スラッシュ区切りのブランチ名は階層として整理し、スマホでも辿りやすい構成にしています。
        </p>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a GitHub Pages hub for branch index.html files."
    )
    parser.add_argument(
        "--output-dir",
        required=True,
    ...
    parser.add_argument(
        "--repository",
        default="",
        help="Repository name used in the page title.",
    )
